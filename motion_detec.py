        ...
        try:
            ...
            while self.running.is_set() and (time.time() - start_time < self.duration):
                try:
                    ...
                    data_resized = cv2.resize(data[:, :], (self.w, self.h))
...
while True:
    ...
    roi = thresh_mask[center_y - box_size:center_y + box_size, center_x - box_size:center_x + box_size]
    ...
    if motion_score > trigger_threshold:
        ...
        if not motion_detected:
            ...
            if thermal_thread is None or not thermal_thread.is_alive():
                thermal_thread = ThermalCaptureThread(duration, temp_min, temp_max, w, h, output_folder)
        ...
        if thermal_thread and thermal_thread.is_alive():
            thermal_thread.stop()
            thermal_thread.join()
        thermal_thread = None  # <-- IMPORTANT FIX
